...
const getIconForCategory = (name: string) => {
  ...
  if (normalized.includes('home') || normalized.includes('furniture')) return HomeIcon;
  if (normalized.includes('toy') || normalized.includes('baby')) return Baby;
  if (normalized.includes('cloth') || normalized.includes('apparel')) return Shirt;
  if (normalized.includes('health') || normalized.includes('wellness')) return HeartPulse;
  return Apple; // Default fallback
};

export default function Sidebar({ initialCategories = [] }: { initialCategories?: Category[] }) {
  ...
  return (
    <aside className={`${isOpen ? 'flex' : 'hidden'} w-64 bg-white border-r border-gray-200 flex-col h-full shrink-0 overflow-y-auto`}>
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50 mt-auto">
        <div className="bg-brand-primary/10 rounded-lg p-4">
          <p className="text-sm font-bold text-brand-primary mb-1">Walmart+ Member?</p>
          <p className="text-xs text-gray-600 mb-3">Sign in to see your exclusive offers.</p>
          <button className="w-full bg-brand-primary text-white rounded-full py-2 text-sm font-bold hover:bg-brand-secondary transition-colors">
            Sign In
          </button>
        </div>
      </div>
